...
class BlockImageDiff(object):
  ...
  def DelFrontSameFreeStashs(self, prefix):
    f = open(prefix,'r')
    allLines = f.readlines()
    f.close()
    exitSameFreeStash = "false"
    for eachLine in allLines:
      if eachLine[0:4] == 'free':
        count = allLines.count(eachLine)
        if count > 1:
          print("The same free stash: %s" % (eachLine,))
          exitSameFreeStash = "true"
          for i in range(count-1):
            del allLines[allLines.index(eachLine)]

    if exitSameFreeStash == "true":
      fobj = open(prefix, 'w')
      for eachKey in allLines:
        fobj.write("%s" % (eachKey))
      fobj.close()
  ...
  def WriteTransfers(self, prefix):
    ...
    with open(prefix + ".transfer.list", "wb") as f:
      for i in out:
        f.write(i)
    self.DelFrontSameFreeStashs(prefix + ".transfer.list")
      ...
      def diff_worker():
        while True:
          ...
          try:
            ...
            print("ERROR:try to compute_patch for %s fail" % (xf.tgt_name))
            raise e
          size = len(patch)
          with lock:
